...
int load_frame(void *fp, float* x, float* v, int N, float *L){
  ...
  // fscanf --> Toma un input (en este caso toma un archivo) y se lo asigna a la variable frame.
  // fscanf --> Agarras un valor de fp y se lo asignas a &frame.
  // & (ampersand) es la inversa del *() --> Con el *()vas a la posicion de memoria y ves que tiene.
  // con el &frame vas a la posicion de memoria y ves que hay.
