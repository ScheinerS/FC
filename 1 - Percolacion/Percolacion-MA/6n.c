...
{ float p;
  ...
  n_sn = malloc(sizeof("./gamma/gamma_00000_dim_000"));
  ...
  *(dims+5) = 128;
  ...
  { p = 0.02;
    ...
    red = malloc((dim*dim+1)*sizeof(int));    //Reserva el espacio necesario para la red.
    s = malloc((dim*dim+1)*sizeof(int));
    ns = malloc((dim*dim+1)*sizeof(int));
    ns_p = malloc((dim*dim+1)*sizeof(double));

      for(j=0;j<dim*dim+1;j++)
      { *(ns_p+j) = 0;
      }
    ...
    { for(j=0;j<dim*dim+1;j++)
      ...
      for(j=1; j<dim*dim+1; j++)
      {
        *(ns_p+j) /= semm;
      }

      for(t=1;t<dim*dim+1;t++)
      { sprintf(n_sn, "./gamma/gamma_%i_dim_%i",t,dim);
        n_s = fopen(n_sn, "a");
        fprintf(n_s,"%f %f\n", p, *(ns_p+t));
        fclose(n_s);
      }
